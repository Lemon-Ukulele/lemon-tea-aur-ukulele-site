...
const BASE_AUDIO_DIR = path.join(ROOT, 'site_audio_from_ukebuddy', 'scales');
...
const URL = 'https://ukebuddy.com/ukulele-scales-ub1';
...
async function getScaleNoteEvents(page) {
  ...
  try {
    ...
    for (const key of keys) {
      ...
      for (const type of types) {
        ...
        const noteEvents = await getScaleNoteEvents(page);
